...
def get_stock_daily(pro, codes, start_date, end_date, batch_size=15):
...
def process_failed_batches():
    ...
    try:
        ...
        print(f"\n发现 {len(failed_lines)} 条失败记录，是否尝试重新获取？(y/n)")
        ...
        for i, line in enumerate(failed_lines):
            try:
                ...
                # 提取批次代码和日期范围
                batch_part = parts[0].split(':')
                if len(batch_part) < 2:
                    print(f"无法提取批次代码: {line}")
                    continue
                
                batch_codes_str = batch_part[1].strip()
                batch_codes = [code.strip() for code in batch_codes_str.split(',')]
                
                # 提取日期范围
                date_part = parts[1].split(':')
                if len(date_part) < 2:
                    print(f"无法提取日期范围: {line}")
                    continue
                
                date_range = date_part[1].strip().split('-')
                if len(date_range) != 2:
                    print(f"日期范围格式错误: {date_part[1]}")
                    continue
